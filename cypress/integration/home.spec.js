describe('Ototrack', function(){
	...
	it ("Should Redirect To ehicles", function(){
	...
	it("Vehicle Page Should Display Vehicles", function(){
		...
		// 	}
		// })
		//cy.get('[data-cy=previous-page]').should('not.have.class', 'disabled').should('be.visible')
	...
	it("Should fill vehicle with imed, plate_no and device_gsm_no but all other empty", function(){
	...
	it("Should fill vehicle with invalid entity", function(){
		...
		cy.get('[data-cy=organizationIdCreate]').should('not.have.length',0)
		...
		cy.get('[data-cy=customerIdCreate]').should('not.have.length',0)
		...
		cy.get("input[data-cy='guarantee_deadline']").click().then (input => {
			input[0].dispatchEvent(new Event('input', { bubbles: true }))
			input.val('2018-12-30T00:32:10.12')
		...
		it("Should Fill Vehicle With plate_no or imed or device_gsm empty or all empty", function(){
